...
void ReverseImageList(ILIST *list)
{
	...
	/* Create the new reverse list */
	ILIST reverse = CreateImageList();

	/* Move over first entry */
	current = list->Last;		/* start at the end of the list */

	current->List = reverse;	/* move current entry to the new list */
	reverse->First = current;	/* Update new list First */

	reverse->Length++;		/* Increment Length */
	...
	list->Length--;			/* Decrement original list length */
	list->Last = current->Next;	/* current->Prev has been swapped so the next entry is current->Next */

	/* Iterate through the list (except for the first and last entry) */
	while((list->First != list->Last) && (list->Last))	/* checks if there is one entry left or for case of list with one entry */
	{
		current = list->Last;		/* start at the end of the list */
	...
	/* Move over the last entry */
	current = list->Last;		/* start at the end of the list */

	current->List = reverse;	/* move current entry to the new list */
	reverse->Last = current;	/* Update new list Last */
