...
{
	unsigned int R_new[WIDTH][HEIGHT];
	unsigned int G_new[WIDTH][HEIGHT];
	unsigned int B_new[WIDTH][HEIGHT];
	...
	for (int i = 0; i < WIDTH; i++)			/* Iterates columns */
	{
		...
		{			
			...
			if ( (i != 0) && (i != (WIDTH - 1)) && (j != 0) && (j != (HEIGHT - 1)) )
			{
				...
				R_new[i][j] = 8 * R[i][j];		/* Multiplies every RGB value by 8 */
				G_new[i][j] = 8 * G[i][j];
				B_new[i][j] = 8 * B[i][j];
				...
				for (int k = -1; k < 2; k++)		/* Iterates through all surrounding pixels */
				{
					for (int l = -1; l < 2; l++)
					{
						...
						R_new[i][j] -= R[i + k][j + l];		/* Subtracts the surrounding RGB values */
						G_new[i][j] -= G[i + k][j + l];
						B_new[i][j] -= B[i + k][j + l];
			...
			else	/* edge pixel case */
			{
				R_new[i][j] = 0;	/* Sets the edge pixels to black */
				G_new[i][j] = 0;
				B_new[i][j] = 0;
			}
